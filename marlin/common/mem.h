...
__device__ inline void cp_async4_pred_zfill(void* smem_ptr, const void* glob_ptr, bool pred = true, const bool zfill = false) {
  const int BYTES = 16;
  ...
  asm volatile(
    ...
    "}\n" :: "r"((int) pred), "r"(smem), "l"(glob_ptr), "n"(BYTES), "r"(src_in_bytes)
  );
}
