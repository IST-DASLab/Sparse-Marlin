...
__device__ inline void mma_t(const FragB& a_frag0, const FragB& a_frag1, const FragA& frag_b, FragC& frag_c) {
...
// m16n8k32 sparse tensor core mma instruction with fp16 inputs and fp32 output/accumulation.
__device__ inline void mma_sp(const FragB& a_frag0, const FragB& a_frag1, const FragA& frag_b, FragC& frag_c, FragM& frag_m, const int psel) {
  const uint32_t* a0 = reinterpret_cast<const uint32_t*>(&a_frag0);
  const uint32_t* a1 = reinterpret_cast<const uint32_t*>(&a_frag1);
  const uint32_t* b = reinterpret_cast<const uint32_t*>(&frag_b);
  ...
  if(psel==0){
    ...
    asm volatile(
      ...
      :"=f"(c[4]), "=f"(c[5]), "=f"(c[6]), "=f"(c[7])
      : "r"(a0[0]),  "r"(a1[0]),  "r"(a0[1]),  "r"(a1[1]),
        "r"(b[1]),  "r"(b[3]),  "r"(b[5]),  "r"(b[7]),
        "f"(c[4]),  "f"(c[5]),  "f"(c[6]),  "f"(c[7]),  "r"(e[0])
      );
  } else {
    asm volatile(
      "mma.sp.sync.aligned.m16n8k32.row.col.f32.f16.f16.f32 "
...
__device__ __forceinline__ uint2 to_half4(float c0, float c1, float c2, float c3)
{
    uint2 r;
    asm("{\n\t"
        ".reg .f16 a, b, c, d; \n\t"
        ...
        "mov.b32 %0, {a, b};   \n\t"
        "mov.b32 %1, {c, d};   \n\t"
        "}" : "=r"(r.x), "=r"(r.y) : "f"(c0),"f"(c1), "f"(c2),"f"(c3));
    return r;
}
