...
__global__ void Marlin_2_4(
  ...
  const int4* __restrict__ meta, // 2bit metadata information about 2:4 format on B
  ...
  int k_tiles = prob_k / 32 / thread_k_blocks; // number of thread_k_blocks in k-dim
  int n_tiles = prob_n / 16 / thread_n_blocks; // number of thread_n_blocks in n-dim
  int iters = ceildiv(k_tiles * n_tiles * parallel, gridDim.x); // iters needeed to cover all slices
  ...
  constexpr int a_sh_stride = 32 * thread_k_blocks / 8; // stride of an A matrix tile in shared memory
  constexpr int a_gl_rd_delta_o = 32 * thread_k_blocks / 8; // delta between subsequent A tiles in global memory
  ...
  constexpr int a_sh_rd_delta_o = 4 * ((threads / 32) / (thread_n_blocks / 4)); // between shared memory tile reads //RLC: 2 * #warps k-dim
  ...
  constexpr int m_sh_stride = (16*thread_n_blocks) / 4; // #warps n-dim * threads/warp
  ...
  int m_gl_rd = m_gl_stride * (threadIdx.x / (m_sh_stride)) + (threadIdx.x % (m_sh_stride));
  ...
  for (int i = 0; i < b_sh_wr_iters; i++) {
    ...
    for (int j = 0; j < thread_m_blocks; j++){
      a_sh_rd_trans[0][i][j] = transform_a(a_sh_rd_delta_o * i + a_sh_rd_delta_i * j + a_sh_rd);
      a_sh_rd_trans[1][i][j] = transform_a(a_sh_rd_delta_o * i + a_sh_rd_delta_i * j + a_sh_rd + 2);
    }
  }
  ...
  auto fetch_to_shared = [&] (int pipe, int a_off, bool pred = true) {
    if (pred) {
      ...
      for (int i = 0; i < m_sh_iters; i++) {
        if(m_sh_wr_pred)
          cp_async4_stream(&sh_meta_stage[m_sh_wr_delta * i + m_sh_wr], meta_ptr[i]);
  ...
  auto fetch_to_registers = [&] (int k, int pipe) {
    ...
    for (int i = 0; i < thread_m_blocks; i++){
      ldsm4(frag_a[k % 2][i][0], &sh_a_stage[a_sh_rd_trans[0][k % b_sh_wr_iters][i]]);
      ldsm4(frag_a[k % 2][i][1], &sh_a_stage[a_sh_rd_trans[1][k % b_sh_wr_iters][i]]);
    }
    ...
    ldsm4_m(frag_m[k % 2][0], &sh_m_stage[m_sh_rd_delta * (k % m_sh_iters) + m_sh_rd]);
  };
  ...
  auto matmul = [&] (int k) {
    ...
    for (int j = 0; j < 4; j++) {
      ...
      #pragma unroll
      for (int i = 0; i < thread_m_blocks; i++) {
        mma_sp(frag_b0, frag_b1, frag_a[k % 2][i][0], frag_c[i][j][0], frag_m[k % 2][j/2], j%2);
      }
    }
  };
  ...
  auto global_reduce = [&] (bool first = false, bool last = false) {
    ...
    if (threadIdx.x < active_threads) {
      ...
      int c_gl_wr_delta_i = c_gl_stride; //8 threads (e.g., 0,4,8,12,16,20,24,28)
      int c_gl_wr = 2 * c_gl_stride * (threadIdx.x % 4) + 8 * (threadIdx.x / 32) + (threadIdx.x % 32) / 4;
      ...
      if (!first) {
        ...
        for (int i = 0; i < thread_m_blocks * 4; i++) {
          cp_async4_pred(
            ...
            i < (thread_m_blocks - 1) * 4 || 8 * (i / 2) + col + (i%2) < prob_m
          );
        }
        cp_async_fence();
        cp_async_wait<0>();
      ...
      #pragma unroll
      for (int i = 0; i < thread_m_blocks * 4; i++) {
        if (i < (thread_m_blocks - 1) * 4 || 8 * (i / 2) + col + (i%2) < prob_m) {
          if (!first) {
            ...
            for (int j2 = 0; j2 < 2; j2++) {
              ...
              for (int j1 = 0; j1 < 4; j1++) {
                reinterpret_cast<float*>(&frag_c)[4 * 2 * 4 * (i / 4) + 8 * j1 + 2 * j2 + 4*((i%4)/2) + i%2] += __half2float(
          ...
          if (!last) {
            ...
            for (int j2 = 0; j2 < 2; j2++) {
              #pragma unroll
              for (int j1 = 0; j1 < 4; j1++) {
                reinterpret_cast<__half*>(&c)[(j2*4+j1)] = __float2half(
                    reinterpret_cast<float*>(&frag_c)[4 * 2 * 4 * (i / 4) + 8 * j1 + 2 * j2 + 4*((i%4)/2) + i%2]
                  );
              }
            }
  ...
  auto write_result = [&] () {
    ...
    int c_sh_wr = c_sh_stride_2 * ((threadIdx.x % 32) % 4) + ((threadIdx.x % 32) / 4); //RLC:
    c_sh_wr += 8 * (threadIdx.x / 32); // 128/4(half4)

    constexpr int c_sh_rd_delta = c_sh_stride_3 * (threads / (2 * 2 * thread_n_blocks)); //RLC:
    int c_sh_rd = c_sh_stride_3 * (threadIdx.x / (2 * 2 * thread_n_blocks)) + (threadIdx.x % (2 * 2 * thread_n_blocks));
    ...
    if (threadIdx.x / 32 < thread_n_blocks / 4) { //RLC:  only warp 0 and 1 baseline example
        #pragma unroll
        for (int i = 0; i < thread_m_blocks; i++) {
          int wr = c_sh_wr;
          write(wr,                                       frag_c[i][0][0][0], frag_c[i][1][0][0], frag_c[i][2][0][0], frag_c[i][3][0][0], frag_s[0][0],
                                                          frag_c[i][0][0][2], frag_c[i][1][0][2], frag_c[i][2][0][2], frag_c[i][3][0][2], frag_s[0][2]);
          //if((col+1)<prob_m){
            write(wr + c_sh_stride,                         frag_c[i][0][0][1], frag_c[i][1][0][1], frag_c[i][2][0][1], frag_c[i][3][0][1], frag_s[0][0],
                                                            frag_c[i][0][0][3], frag_c[i][1][0][3], frag_c[i][2][0][3], frag_c[i][3][0][3], frag_s[0][2]);
            //if((col+8)<prob_m){
              write(wr + 4*c_sh_stride_2,                     frag_c[i][0][1][0], frag_c[i][1][1][0], frag_c[i][2][1][0], frag_c[i][3][1][0], frag_s[0][0],
                                                              frag_c[i][0][1][2], frag_c[i][1][1][2], frag_c[i][2][1][2], frag_c[i][3][1][2], frag_s[0][2]);
              //if((col+9)<prob_m){
                write(wr + 4*c_sh_stride_2 + c_sh_stride,       frag_c[i][0][1][1], frag_c[i][1][1][1], frag_c[i][2][1][1], frag_c[i][3][1][1], frag_s[0][0],
                                                                frag_c[i][0][1][3], frag_c[i][1][1][3], frag_c[i][2][1][3], frag_c[i][3][1][3], frag_s[0][2]);
              //}
            //}
          //}
...
#define CALL_IF_2_4(THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS, GROUP_BLOCKS) \
  else if ( \
    thread_m_blocks == THREAD_M_BLOCKS && thread_n_blocks == THREAD_N_BLOCKS && thread_k_blocks == THREAD_K_BLOCKS && \
    group_blocks == GROUP_BLOCKS \
  ) { \
    cudaFuncSetAttribute( \
      Marlin_2_4<THREADS, THREAD_N_BLOCKS, THREAD_M_BLOCKS, THREAD_K_BLOCKS, STAGES, GROUP_BLOCKS>, \
      cudaFuncAttributeMaxDynamicSharedMemorySize, \
      SHARED_MEM \
    ); \
    Marlin_2_4< \
      THREADS, THREAD_N_BLOCKS, THREAD_M_BLOCKS, THREAD_K_BLOCKS, STAGES, GROUP_BLOCKS \
    ><<<blocks, THREADS, SHARED_MEM, stream>>>( \
      A_ptr, B_ptr, meta_ptr, C_ptr, s_ptr, \
...
int marlin_cuda_2_4(
  ...
  int blocks = sms;
  if (prob_m % thread_m != 0 || prob_k % thread_k != 0 || (group_blocks != -1 && (prob_k/2) % group_blocks != 0))
